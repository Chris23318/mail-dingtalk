    ...
    def _call_api(q: str):
        ...
        try:
            ...
            if "trans_result" in data:
                ...
                # 如果翻译结果和原文基本一致，说明翻译无意义，视为失败
                if result.strip().lower() == q.strip().lower():
                    print("[D] 翻译结果与原文相同，跳过")
                    return None
    ...
    for mid in mail_ids:
        ...
        if ENABLE_TRANSLATE:
            ...
            if not is_chinese(body):
                ...
                print("[D] 正文已是中文，跳过翻译")
            print(f"[D] subject_cn={'有' if subject_cn else '无'}, body_cn={'有' if body_cn else '无'}")
