...
SENT_IDS_FILE    = os.environ.get("SENT_IDS_FILE", "sent_ids.txt")
# ============================================================
...
def strip_quoted_reply(text: str) -> str:
    ...
    for i, line in enumerate(lines):
        ...
        # 2. 经典中文引用分隔："在 xxx, xxx 写道："
        if re.match(r'^在\s.*\d{4}.*(写道|写道：|wrote)', s):
            return "\n".join(lines[:i]).strip()

        # 3. 经典英文引用分隔："On xxx, xxx wrote:"
        if re.match(r'^On\s.*,.*(wrote|said|schrieb):', s, re.IGNORECASE):
            return "\n".join(lines[:i]).strip()

        # 4. 中文邮件转发头连续出现（发件人/发送时间/收件人/主题）
        if re.match(r'^(发件人|发送时间|收件人|抄送|主题|From|Sent|To|Cc|Subject|Date)[：:]', s, re.IGNORECASE):
            header_count = 1
            for j in range(i + 1, min(i + 4, len(lines))):
                ns = lines[j].strip()
                if re.match(r'^(发件人|发送时间|收件人|抄送|主题|From|Sent|To|Cc|Subject|Date)[：:]', ns, re.IGNORECASE):
                    header_count += 1
                else:
                    break
            if header_count >= 2:
    ...
    if os.path.exists(SENT_IDS_FILE):
        with open(SENT_IDS_FILE, "r", encoding="utf-8") as f:
            for line in f:
                ...
                if line:
                    ids.add(line)
    print(f"[*] 已发送记录: {len(ids)} 条")
    return ids


def save_sent_ids(new_ids):
    """追加新发送的 Message-ID 到去重文件"""
    if not new_ids:
        return
    with open(SENT_IDS_FILE, "a", encoding="utf-8") as f:
        for mid in new_ids:
            f.write(mid + "\n")
    print(f"[*] 新增发送记录: {len(new_ids)} 条")


def fetch_today_emails(sent_ids=None):
    if sent_ids is None:
        sent_ids = set()
...
def main():
    ...
    try:
        emails, new_ids = fetch_today_emails(sent_ids)
