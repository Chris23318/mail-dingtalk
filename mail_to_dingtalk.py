...
def translate_to_chinese(text: str):
    ...
    def _call_api(q: str):
        ...
        sign = hashlib.md5((BAIDU_APPID + q + salt + BAIDU_KEY).encode()).hexdigest()
        params = urllib.parse.urlencode({
            "q": q, "from": "auto", "to": "zh",
            "appid": BAIDU_APPID, "salt": salt, "sign": sign,
        })
        try:
            with urllib.request.urlopen(API_URL + "?" + params, timeout=10) as resp:
                data = json.loads(resp.read().decode())
            if "trans_result" in data:
                return "".join(d["dst"] for d in data["trans_result"])
            print(f"[!] 百度翻译错误: {data}")
            return None
        except Exception as exc:
            print(f"[!] 百度翻译请求失败: {exc}")
            return None
